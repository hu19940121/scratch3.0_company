...
const mapStateToProps = state => ({
    // profileUrl: state.session && state.session.session && state.session.session.user ?
    //     `/users/${state.session.session.user.username}` : '',
    thumbnailUrl: state.scratchGui.login.session && state.scratchGui.login.userInfo ?
        state.scratchGui.login.userInfo.avatarUrl : null,
    username: state.scratchGui.login.session && state.scratchGui.login.userInfo ?
        state.scratchGui.login.userInfo.nickName : ''
});
